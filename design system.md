...
### ב. כרטיס מידע פיננסי (Financial Stat Card)

כרטיס נקי המציג נתון מספרי עם אינדיקטור צדדי דק בצבעי המערכת.
...
```
### ג. שדה קלט מודרני (Input Field)
שדה קלט נקי ומרווח עבור מסכי הלוגין והטפסים במערכת.
...
```
## 4. עקרונות פריסה, ריווח ואנימציה (Layout & Animation Rules)
